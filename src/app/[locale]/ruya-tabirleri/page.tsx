...
export default async function Page(props: { params: Promise<{ locale: Locale }>; searchParams: Promise<{ letter?: string; page?: string }> }) {
    ...
    if (letter) {
        redirect(`/${locale}/ruya-tabirleri/${letter.toLowerCase()}`);
    }

    // Redirect or return 404 if locale doesn't match the path segment
    if (locale !== 'tr') {
    ...
    return (
        <Suspense fallback={<div className="min-h-screen bg-black flex items-center justify-center text-white font-black uppercase tracking-[0.3em]">Yükleniyor...</div>}>
            <DreamArchiveDisplay
                dreams={dreams}
                locale={locale}
                dict={dict}
